...
public class TestPassedOutPutXMLParserHandler {
  ...
  public void testParsedStructure() throws Exception {
    ...
    assertNotNull(suite);
    assertEquals("TestSYSDirFunctions.t", suite.getSafeName());
    assertEquals(2, suite.getTotalCount());
    assertEquals(2, suite.getChildren().size());

    test = suite.getChildTestByName("TestCase1");
    assertNotNull(test);
    assertEquals("TestCase1", test.getSafeName());
    assertEquals(1, test.getTotalCount());

    test = suite.getChildTestByName("TestCase2");
    assertNotNull(test);
    assertEquals("TestCase2", test.getSafeName());
    assertEquals(1, test.getTotalCount());
  }
  ...
  public void testPassedTestRun() throws Exception {
    ...
    SCTMTestSuiteResult suite = rootSuite.getChildSuiteByName("4Test.pln").getChildSuiteByName("SubSuite.pln")
        .getChildSuiteByName("TestSYSDirFunctions.t");
  ...
  public void testNormalizedNUnitSuiteName() throws Exception {
    ...
    assertNotNull(suite);
    assertEquals("SilkTest.Ntf.Test.Awt", suite.getSafeName());
